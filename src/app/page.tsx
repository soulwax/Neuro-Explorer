    ...
    <div className="space-y-6">
      <section className="rounded-[32px] border border-white/10 bg-white/6 p-6 shadow-[0_18px_50px_rgba(3,10,20,0.24)] backdrop-blur sm:p-8">
        ...
        <p className="mt-4 max-w-3xl text-sm leading-7 text-slate-300 sm:text-base">
          Deterministic simulations, server-backed labs, and Cloudflare
          AI-powered experiences now share the same App Router shell and
          internal <code>/api/*</code> contract.
        </p>
        <p className="mt-3 text-sm font-medium text-amber-100">
          {moduleCount} interactive modules are now served from the primary Next.js app.
        </p>
      ...
      <section className="rounded-[28px] border border-white/10 bg-white/6 p-5 backdrop-blur">
        ...
        <p className="mt-3 max-w-3xl text-sm leading-7 text-slate-300">
          This is intentionally not a Prisma/Auth/tRPC reset. The important move
          was consolidating the product around App Router pages and internal API
          route handlers while keeping the simulation engines in TypeScript.
        </p>
        <p className="mt-3 max-w-3xl text-sm leading-7 text-slate-300">
          `Vision` and `Ask` still rely on Cloudflare AI, but they now follow
          the same internal API contract as the rest of the site and ship with
          the same Next.js deployment unit.
        </p>
      </section>
    </div>
  );
