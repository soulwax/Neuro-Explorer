...
export function VisionExplorer() {
	...
	const [consultCaseId, setConsultCaseId] = useState<string>(visionConsultCases[0]!.id);
	...
	const activeConsultCase = visionConsultCases.find((item) => item.id === consultCaseId) ?? visionConsultCases[0]!;
	const correlatedStage =
		pipeline.find((stage) => activeConsultCase.pipelineCorrelation.includes(stage.corticalArea.split(' ')[0]!)) ?? null;
		...
		<div className="space-y-6">
			<section className="rounded-[28px] border border-white/10 bg-white/6 p-5 shadow-[0_16px_48px_rgba(3,10,20,0.22)] backdrop-blur">
				<div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
					<div>
						...
						<h1 className="mt-2 text-3xl font-semibold tracking-tight text-white sm:text-4xl">Visual cortex inference</h1>
						<p className="mt-4 max-w-3xl text-sm leading-7 text-slate-300">
							The page still runs image classification through Cloudflare AI, but it now doubles as a post-clinical visual localization lab:
							field-cut logic, ventral-stream syndromes, and the next data that should settle the case.
						</p>
					</div>
			...
			<section className="rounded-[28px] border border-white/10 bg-white/6 p-5 backdrop-blur">
				...
				<div className="mt-6 grid gap-6 xl:grid-cols-[minmax(0,1.05fr)_340px]">
					<div>
						<p className="text-xs uppercase tracking-[0.24em] text-slate-400">Ventral stream pipeline</p>
					...
					<div className="rounded-[28px] border border-white/10 bg-slate-950/35 p-5">
						...
						<p className="mt-4 text-sm leading-6 text-slate-300">{visionKeyInsight}</p>
					</div>
				</div>
			</section>
			...
			<section className="grid gap-6 xl:grid-cols-[minmax(0,1.05fr)_340px]">
				...
				<div className="rounded-[28px] border border-white/10 bg-white/6 p-5 backdrop-blur">
					<p className="text-xs uppercase tracking-[0.24em] text-slate-400">Skip connections</p>
					<h2 className="mt-1 text-xl font-semibold text-white">Residual paths as cortical feedback</h2>
			...
			<section className="rounded-[28px] border border-white/10 bg-white/6 p-5 backdrop-blur">
				<div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
					<div>
						<p className="text-xs uppercase tracking-[0.24em] text-slate-400">Post-clinical visual syndromes</p>
						<h2 className="mt-1 text-xl font-semibold text-white">Read the visual complaint before you chase the image label</h2>
						<p className="mt-4 max-w-3xl text-sm leading-7 text-slate-300">
							These cases force the same move good consults make: define the syndrome, pick the strongest localization layer, reject the
							neat but weaker alternative, and ask for the next datum that should settle the case.
						</p>
					</div>
				</div>
				...
				<div className="mt-5 grid gap-6 xl:grid-cols-[minmax(0,1.05fr)_340px]">
					<div className="rounded-[24px] border border-white/10 bg-slate-950/35 p-5">
						...
						<h3 className="mt-2 text-lg font-semibold text-white">{activeConsultCase.title}</h3>
						<p className="mt-4 text-sm leading-7 text-slate-300">{activeConsultCase.syndromeFrame}</p>
						...
						<div className="mt-5 grid gap-4 lg:grid-cols-2">
							<div className="rounded-[20px] border border-white/10 bg-white/6 p-4">
								...
								<p className="mt-2 text-sm font-medium text-white">{activeConsultCase.strongestLocalization}</p>
								<p className="mt-3 text-sm leading-7 text-slate-300">{activeConsultCase.whyItFits}</p>
							</div>
							<div className="rounded-[20px] border border-white/10 bg-white/6 p-4">
					...
					<div className="rounded-[24px] border border-white/10 bg-slate-950/35 p-5">
						<p className="text-xs uppercase tracking-[0.18em] text-slate-400">Pipeline correlation</p>
						<p className="mt-3 text-sm leading-7 text-slate-300">{activeConsultCase.pipelineCorrelation}</p>
						{correlatedStage ? (
							<div className="mt-4 rounded-[20px] border border-cyan-300/20 bg-cyan-300/8 p-4">
								<p className="text-xs uppercase tracking-[0.18em] text-cyan-100">Closest modeled stage</p>
								<p className="mt-2 text-sm font-medium text-white">{correlatedStage.corticalArea}</p>
								<p className="mt-3 text-sm leading-7 text-slate-300">{correlatedStage.biology}</p>
							</div>
						) : null}
				...
				<div className="mt-6">
					<CompareShell
						...
						left={
							<>
								<p className="font-semibold text-white">{activeConsultCase.strongestLocalization}</p>
								<p className="mt-3">{activeConsultCase.whyItFits}</p>
							</>
						}
						right={
							<>
								<p className="font-semibold text-white">{activeConsultCase.weakerAlternative}</p>
			...
			<section className="rounded-[28px] border border-white/10 bg-white/6 p-5 backdrop-blur">
				<p className="text-xs uppercase tracking-[0.24em] text-slate-400">Biology notes</p>
				<h2 className="mt-1 text-xl font-semibold text-white">Why each stage matters</h2>
				<div className="mt-5 grid gap-4 lg:grid-cols-2">
					{pipeline.map((stage) => (
						<div key={`${stage.corticalArea}-biology`} className="rounded-[24px] border border-white/10 bg-slate-950/35 p-4">
